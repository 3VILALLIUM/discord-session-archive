...
- `.githooks/pre-commit` runs git identity, PR action policy, and privacy checks before commit.
- `.githooks/pre-push` runs git identity, PR action policy, privacy checks, and tests before push, and blocks direct pushes to `origin/main`.
- `scripts/git_identity_guard.ps1` / `scripts/git_identity_guard.sh` block unsafe git identity configuration and author metadata; bash pre-push checks allow only the approved repo identity or GitHub's noreply committer metadata.
