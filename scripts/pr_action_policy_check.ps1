...
if ($missing.Count -gt 0 -or $forbidden.Count -gt 0 -or $unexpected.Count -gt 0) {
    ...
    Write-Host "AGENTS.md must keep the read-only audit allowance plus hard completed-review, conversation, and close/MERGE gates in the PR Review Gate section."
...
Write-Host "PR action policy check passed: read-only audit allowance plus completed-review, conversation, and close/MERGE gates are present."
